...
export class HalfedgeMesh {
  ...
  // The following four fields are the key fields to
  // represent half-edge based meshes.
  verts: Vertex[];
  edges: Edge[];
  faces: Face[];
  halfedges: Halfedge[];
  ...
  constructor(data: string) {
    ...
    for (let line of lines) {
      ...
      switch (tokens[0].trim()) {
        case 'v':
          ...
          break;
        case 'vt':
          uvs.push(new Vector(parseFloat(tokens[1]), parseFloat(tokens[2]), 0));
          break;
        case 'f':
          ...
          for (let i = 1; i < tokens.length; i++) {
            ...
            uvindices.push(parseInt(vv[1]) - 1);
          }
          break;
      }
    }
    ...
    this.halfedges = [];
    this.boundaries = [];
    this.buildMesh(indices, positions, uvindices, uvs);
  }

...
export default class Matrix {
  ...
  /**
   * mulvec computes Matrix and Vector multiplication.
   * @param {Vector} v is the given vector for multiplication
   * @return {Vector} the result of matrix vector multiplication
   */
  mulvec(v) {
    const x = this.x00*v.x + this.x01*v.y + this.x02*v.z + this.x03*v.w
    const y = this.x10*v.x + this.x11*v.y + this.x12*v.z + this.x13*v.w
    const z = this.x20*v.x + this.x21*v.y + this.x22*v.z + this.x23*v.w
    const w = this.x30*v.x + this.x31*v.y + this.x32*v.z + this.x33*v.w

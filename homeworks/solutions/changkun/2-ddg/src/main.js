  ...
  updateCurvature() {
    ...
    this.internal.mesh.vertices.forEach(v => {
      ...
      if (this.params.curvatureMethod === 'none') {
        ...
        // The color encoding defines the quality of our final visualization.
        // The majority of the curvature is concentrated except for a few outliers
        // (because of the triangulation, Delaunay does not produce this issue).
        // An empirical rule of truncated bound is to put everything
        // in [-Math.min(Math.PI/8, max), Math.min(Math.PI/8, max)].
        // Note that this will require the calculated curvature to be
        // rescaled in this domain.

...
export class AABB {
  ...
  constructor(vs: Vertex[]) {
    ...
    for (let i = 0; i < vs.length; i++) {
      this.min.x = Math.min(this.min.x, vs[i].pos.x);
      this.min.y = Math.min(this.min.y, vs[i].pos.y);
      this.min.z = Math.min(this.min.z, vs[i].pos.z);
      this.max.x = Math.max(this.max.x, vs[i].pos.x);
      this.max.y = Math.max(this.max.y, vs[i].pos.y);
      this.max.z = Math.max(this.max.z, vs[i].pos.z);
    }
  }

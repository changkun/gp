...
```
Blenders internal data structure is called BMesh. A BMesh stores Faces,Loops,Edges and Vertices for each Mesh (Main Element structures). Each Loop corresponds to an edge, but there can be more than one loop per edge. Each Loop stores how these Main element structures are connected, and therefore a BMesh can represent also non-manifolds. A halfedge however can only represent manifold meshes.

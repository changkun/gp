...
export default class Renderer {
  ...
  sceneLeft: Scene;
  sceneRight: Scene;
  cameraMeshLeft: PerspectiveCamera;
  cameraMeshRight: PerspectiveCamera;
  controlsLeft: OrbitControls;
  controlsRight: OrbitControls;
  ...
  constructor() {
    ...
    this.sceneLeft.background = new Color(0x000000);
    ...
    window.addEventListener(
      'resize',
      () => {
        this.cameraMeshLeft.aspect =
          (0.5 * window.innerWidth) / window.innerHeight;
        this.cameraMeshLeft.updateProjectionMatrix();

        this.cameraMeshRight.aspect =
          (0.5 * window.innerWidth) / window.innerHeight;

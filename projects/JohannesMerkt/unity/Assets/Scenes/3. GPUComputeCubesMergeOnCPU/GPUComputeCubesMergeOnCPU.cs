...
[ExecuteAlways]
public class GPUComputeCubesMergeOnCPU : MonoBehaviour
{
    ...
    void Initialize() {
        ...
        cubesBuffer = new ComputeBuffer(cubesPerSide * cubesPerSide * cubesPerSide, ((sizeof(float) * 6) + sizeof(int)) * 12 + sizeof(int) * 3 * 5 + sizeof(int));
                ...
                for (int j = 4; j < 16; j++) {
                    if (bits[j]) {
                        ...
                        if (!vertMap.ContainsKey(vert.index)) {
                            ...
                            normals[vertMap[vert.index]] += vert.normal;
                        }
                    }
                }
                // faces
